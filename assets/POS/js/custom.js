		...
		$.ajax({
			...
			success:function(response) {
				...
				$.each(JSON.parse(response.sale_vat_objects), function(key, value){
				    total_vat_section_to_show += '<span class="tax_field_order_details" id="tax_field_order_details_'+value.tax_field_id+'">'+value.tax_field_type+'</span>: '+currency+' <span class="tax_field_amount_order_details" id="tax_field_amount_order_details_'+value.tax_field_id+'">'+value.tax_field_amount+'</span><br/>';
				});
...
/*	$(document).on('mouseleave','.header_part_left_left .outlet_holder .outlet_holder_moving',function(){
	...
	$('#given_amount_input').on('keyup',function(e){
		...
		if (e.which != 110 && !(e.which<106 && e.which>95) && e.which != 190 && e.which != 16 && e.which != 53 && e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
			$(this).val('');
		}
...
/*	$(document).on('mouseenter','.header_part_left_left .outlet_holder .outlet_holder_moving',function(){
	...
	$(document).on('click','.single_item',function(){
		...
		tax_information = (IsJsonString(tax_information))?JSON.parse(tax_information):'';
		if(tax_information.length>0){
			for(k in tax_information){
				tax_information[k].item_vat_amount_for_unit_item = ((parseFloat(item_price)*parseFloat(tax_information[k].tax_field_percentage))/parseFloat(100)).toFixed(2);
				tax_information[k].item_vat_amount_for_all_quantity = (parseFloat(tax_information[k].item_vat_amount_for_unit_item)*parseFloat(1)).toFixed(2);
			}
		}
		...
		draw_table_for_order += (row_number>0)?'':'<div class="single_order fix" id="order_for_item_'+item_id+'">';
			draw_table_for_order += '<div class="first_portion fix">';
				...
				draw_table_for_order += '<span class="item_vat" id="item_vat_percentage_table'+item_id+'" style="display:none;">'+JSON.stringify(tax_information)+'</span>';
	...
	$('#cancel_button').on('click',function(){
		...
		if(total_items_in_cart>0){
			var r = confirm(cart_not_empty_want_to_clear);
			if (r == true) {
	...
	$(document).on('click','.edit_item',function(){
		...
		if(single_order_element_object.find('#item_modifiers_price_table_'+menu_id).length>0){
			var comma_separeted_modifiers_price = single_order_element_object.find('#item_modifiers_price_table_'+menu_id).html();
			var modifiers_price_array = (comma_separeted_modifiers_price!="")?comma_separeted_modifiers_price.split(","):'';
			modifiers_price_array.forEach(function(modifier_price){
				modifiers_price = (parseFloat(modifiers_price)+parseFloat(modifier_price)).toFixed(2);
			});
			parseFloat(single_order_element_object.find('#item_modifiers_price_table_'+menu_id).html()).toFixed(2);
		}
		...
		var modifiers_price_as_per_item_quantity = (parseFloat(modifiers_price)*parseFloat(item_quantity)).toFixed(2);
		var total_price = (parseFloat(item_price_with_discount)+parseFloat(modifiers_price_as_per_item_quantity)).toFixed(2);
	...
	$(document).on('click','.modal_modifiers',function(){
		...
		var total_modifier_price = parseFloat($('#modal_modifiers_unit_price_variable').html()).toFixed(2);
  ...
  //           })
	...
	$('#modify_order').on('mouseover',function(){
		$('#modify_button_tool_tip').fadeIn();
	});
	$('#direct_invoice').on('mouseover',function(){
		$('#direct_invoice_button_tool_tip').fadeIn();
	});
	$('#direct_invoice').on('mouseleave',function(){
		$('#direct_invoice_button_tool_tip').fadeOut();
	});
	$('#modify_order').on('mouseleave',function(){
		$('#modify_button_tool_tip').fadeOut();
	});
	//tooltip modify order button 
	var modify_order_button = $("#modify_order");
	// var modify_order_button = modify_order_button.position();
  	var modify_order_top = modify_order_button.offset().top - $(document).scrollTop();
  	var modify_order_left = modify_order_button.offset().left - $(document).scrollLeft();
  	var modify_order_width = (parseFloat(modify_order_button.width())+parseFloat(30)).toFixed(2);
  	var modify_order_height = modify_order_button.height();
  	var modify_order_half_height = (parseFloat(modify_order_height)/parseFloat(50)).toFixed(2);
  	var order_tool_tip_top = document.getElementById('modify_order').getBoundingClientRect().top;
	...
	// var direct_invoice_button = direct_invoice_button.position();
  	var direct_invoice_top = direct_invoice_button.offset().top - $(document).scrollTop();
  	var direct_invoice_left = direct_invoice_button.offset().left - $(document).scrollLeft();
  	var direct_invoice_height = direct_invoice_button.height();
  	var direct_invoice_tool_tip_top = (parseFloat(direct_invoice_top)-parseFloat(direct_invoice_height)-parseFloat(10)).toFixed(2);
  	...
  	$('#direct_invoice_button_tool_tip').css('top',direct_invoice_tool_tip_top+'px');
...
/*	if(parseFloat(actual_modal_discount_amount)>parseFloat(item_total_price_without_discount)){
	...
	var modifiers_unit_sum_price = parseFloat($('#modal_modifiers_unit_price_variable').html()).toFixed(2);
	
	//set modifiers price as per item quantity 
	var modifiers_price = (parseFloat(modifiers_unit_sum_price)*parseFloat(item_quantity)).toFixed(2);
	$('#modal_modifier_price_variable').html(modifiers_price);

	//add items and modifiers price
	var all_price = (parseFloat(item_price_after_discount)+parseFloat(modifiers_price)).toFixed(2);
...
function do_addition_of_item_and_modifiers_price(){
	...
	$.each(vat_amount, function(key, value){
	    total_vat_section_to_show += '<span class="tax_field" id="tax_field_'+key.substring(key.indexOf('_')+1)+'">'+key.substr(0, key.indexOf('_'))+'</span>: '+currency+' <span class="tax_field_amount" id="tax_field_amount_'+key.substring(key.indexOf('_')+1)+'">'+value+'</span><br/>';
	});
	...
	total_payable_to_show = '';
	var total_vat_amount = 0;
	$.each(vat_amount, function(key, value){
	    total_vat_amount = (parseFloat(total_vat_amount)+parseFloat(value)).toFixed(2);
	    // total_payable_to_show += '<span class="total_payable_for_tax_field" >Total Payable For '+key+'</span>: <span class="total_payable_for_tax_field_amount">'+total_payable+'</span> '+currency+'<br/>';
	});
	var total_payable = (parseFloat(discounted_sub_total_amount)+parseFloat(total_vat_amount)+parseFloat(delivery_charge_amount)).toFixed(2);
...
function set_all_items_modifiers_amount(){
	$('.order_holder .single_order').each(function(i,obj){
		...
		if($(this).find('#item_modifiers_price_table_'+item_id).length>0){
			...
			var comma_separeted_modifiers_price = $(this).find('#item_modifiers_price_table_'+item_id).html();
			var modifiers_price_array = (comma_separeted_modifiers_price!="")?comma_separeted_modifiers_price.split(","):'';
			modifiers_price_array.forEach(function(modifier_price){
				modifiers_price = (parseFloat(modifiers_price)+parseFloat(modifier_price)).toFixed(2);
			});
			var modifiers_price_as_per_item_quantity = (parseFloat(item_quantity)*parseFloat(modifiers_price)).toFixed(2)
			$(this).find('.fifth_column #item_modifiers_price_table_'+item_id).html(modifiers_price_as_per_item_quantity);
		}
...
function get_total_vat(){
	...
	var customer_state_code = (customer_gst_number!="")?customer_gst_number.substr(0,2):'';
	...
	$('.single_order .first_portion .fifth_column span').each(function(i, obj) {
		...
		var item_total_price = parseFloat($(this).parent().parent().find('.fifth_column span').html()).toFixed(2);
		// var this_item_vat_percentage = parseFloat($(this).parent().parent().find('.item_vat').html()).toFixed(2);
		var tax_information = JSON.parse($(this).parent().parent().find('.item_vat').html());
		...
		if(tax_information.length>0){
			...
			for(k in tax_information){
				if(tax_name.includes(tax_information[k].tax_field_name)){
					if(collect_gst=='Yes' && same_state && tax_information[k].tax_field_name!="IGST"){
						var previous_value = tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id];
						tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id] = (parseFloat(previous_value)+parseFloat((parseFloat(parseFloat(tax_information[k].tax_field_percentage)*parseFloat(item_total_price))/parseFloat(100)))).toFixed(2);						
					}else if(collect_gst=='Yes' && !same_state && tax_information[k].tax_field_name!="SGST"){
						var previous_value = tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id];
						tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id] = (parseFloat(previous_value)+parseFloat((parseFloat(parseFloat(tax_information[k].tax_field_percentage)*parseFloat(item_total_price))/parseFloat(100)))).toFixed(2);
					}
					if(collect_tax=='Yes' && collect_gst!="Yes" && (tax_information[k].tax_field_name!="IGST" && tax_information[k].tax_field_name!="CGST" && tax_information[k].tax_field_name!="SGST")){
						var previous_value = tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id];
						tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id] = (parseFloat(previous_value)+parseFloat((parseFloat(parseFloat(tax_information[k].tax_field_percentage)*parseFloat(item_total_price))/parseFloat(100)))).toFixed(2);
					}
				}else{
						
					// console.log(tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id]);
					if(collect_gst=='Yes' && same_state && tax_information[k].tax_field_name!="IGST"){
						tax_name.push(tax_information[k].tax_field_name);
						tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id] = (parseFloat(parseFloat(tax_information[k].tax_field_percentage)*parseFloat(item_total_price))/parseFloat(100)).toFixed(2);	
					}else if(collect_gst=='Yes' && !same_state && tax_information[k].tax_field_name!="SGST"){
						tax_name.push(tax_information[k].tax_field_name);
						tax_object[""+tax_information[k].tax_field_name+"_"+tax_information[k].tax_field_id] = (parseFloat(parseFloat(tax_information[k].tax_field_percentage)*parseFloat(item_total_price))/parseFloat(100)).toFixed(2);	
					}
	...
	// console.log(tax_object);
	return tax_object;

	//previous vat calculation code
	// var all_item_total_vat_amount = 0;
	// $('.single_order .first_portion .fifth_column span').each(function(i, obj) {
	// 	var this_item_quantity = parseFloat($(this).parent().parent().find('.third_column span').html()).toFixed(2);
	// 	var this_item_price = parseFloat($(this).parent().parent().find('.second_column span').html()).toFixed(2);
	...
	for (var key in response.items) {
		...
		draw_table_for_order += '<div class="single_order fix" id="order_for_item_'+this_item.food_menu_id+'">';
			draw_table_for_order += '<div class="first_portion fix">';
				...
				draw_table_for_order += '<span class="item_vat" id="item_vat_percentage_table'+this_item.food_menu_id+'" style="display:none;">'+this_item.menu_taxes+'</span>';
				draw_table_for_order += '<span class="item_discount" id="item_discount_table'+this_item.food_menu_id+'" style="display:none;">'+this_item.discount_amount+'</span>';
				draw_table_for_order += '<span class="item_price_without_discount" id="item_price_without_discount_'+this_item.food_menu_id+'" style="display:none;">'+this_item.menu_price_without_discount+'</span>';
				draw_table_for_order += '<div class="single_order_column first_column fix"><i style="cursor:pointer;" class="fas fa-pencil-alt edit_item" id="edit_item_'+this_item.food_menu_id+'"></i> <span id="item_name_table_'+this_item.food_menu_id+'">'+this_item.menu_name+'</span></div>';
				draw_table_for_order += '<div class="single_order_column second_column fix">'+currency+' <span id="item_price_table_'+this_item.food_menu_id+'">'+this_item.menu_unit_price+'</span></div>';
...
function get_customer_for_edit(customer_id){
	$.ajax({
		...
		success:function(response) {
			...
			if(collect_gst=="Yes"){
				var gst_no = (response.gst_number==null || response.gst_number=='')?'':response.gst_number;
    ...
    $.ajax({
        ...
        success:function(response) { 
            if (response == 'version_file_true') {
                ...
                var msg = "Install" + underscrore + "via" +
                underscrore + "installer";
